	...
	try:
		for row_index in range( row_start, row_end ):
			for col_index in range(column_start, column_end):
				work_sheet.cell( row = row_index, column = col_index).border = Border( top = Side( border_style='thin', color=colors.BLACK ), left  = Side( border_style='thin', color=colors.BLACK ), right = Side( border_style='thin', color=colors.BLACK ), bottom = Side( border_style='thin', color=colors.BLACK ))	
...
def load_ohf( year, filename ):
	...
	try:
		...
		for row_index in range(2, sht_max_row +1 ):
			#creating list_pay
			if ( (str(sheet_ranges.cell	( row = row_index, column = het_nme_col	).value).lower() != 'Sick Time'.lower() ) and
				(datetime.datetime.date(sheet_ranges.cell( row = row_index, column = strt_dte_col).value) >= date_start_year) and 
			   (datetime.datetime.date(sheet_ranges.cell( row = row_index, column = end_dte_col).value) <= date_end_year) and
			   ( ((sheet_ranges.cell( row = row_index, column = empl_tem_col).value).lower() == 'RU FS UPS'.lower() ) or
			   ((sheet_ranges.cell( row = row_index, column = empl_tem_col).value).lower() == 'RU FS Cooling'.lower()) or
			   ((sheet_ranges.cell( row = row_index, column = empl_tem_col).value).lower() == 'RU FS UPS Regions'.lower()))):
			
				het_line = {}
				
				het_line['het_nme'	] = str(sheet_ranges.cell(row = row_index,column = het_nme_col).value)
				het_line['stts'		] = str(sheet_ranges.cell(row = row_index,column = stts_col).value)
				het_line['empl_id'	] = str(sheet_ranges.cell(row = row_index,column = empl_id_col).value)
				het_line['empl_tm'	] = str(sheet_ranges.cell(row = row_index,column = empl_tem_col).value)
				het_line['nts'		] = str(sheet_ranges.cell(row = row_index,column = nts_col).value)
				het_line['strt_dte'	] = datetime.datetime.date(sheet_ranges.cell(row = row_index,column = strt_dte_col).value)
				het_line['end_dte'	] = datetime.datetime.date(sheet_ranges.cell(row = row_index,column = end_dte_col).value)
				het_line['drtn'		] = (het_line['end_dte']-het_line['strt_dte']+timedelta(days=1)).days
				het_line['entr_dte'	] = datetime.datetime.date(sheet_ranges.cell(row = row_index, column = entr_dte_col).value)
				
				if (het_line['nts'		] 	== 'None') or (het_line['nts'] == '-'):
					het_line['nts'		] 	= u'Без комментария'
				het_list.append(het_line)

		#for line in het_list:
			#print line, '\n'
	...
	try:
					...
					if persone['persone_id'].lower() == line_v['empl_id'].lower():
						for day in line_t['dys_lst']:
							if line_v['het_days_lst'][str(day)] > 0:
								a = u'У сотрудника {nme} на {dte} запланирован тренинг и отпуск.'.format(nme = line_v['empl_ru_shrt_nme'], dte = str(day) )
								errors.append(a)
		...
		for line in het_lst_by_nme:
			...
			if int(line['persone_het_drtn']) < 28 :
				a = u'Сотрудник {name} не до конца запланировал свой отпуск, запланировано дней: {days}'.format( name = line['empl_ru_shrt_nme'],  days = line['persone_het_drtn'])
				#print a
				errors.append(a)
				
			#Check 2weeks duration
			if max(duration_dir) < 14 :
				a = u'Сотрудник {name} не имеет отпуск, продолжительность не менeе 2 недель, максимальный размер непрерывного отпуска: {days} дней '.format( name = line['empl_ru_shrt_nme'],  days = max(duration_dir) )
				#print a
				errors.append(a)
			...
			for het_line in line['het_lst']:
				if (het_line['drtn'] > 14) and (het_line['stts'].lower() != 'Approved'.lower() ):
					a = u'Сотрудник {name} запланировал отпуск продолжительностью {days} дней. Требует дополнительного согласования.'.format( name = line['empl_ru_shrt_nme'],  days = het_line['drtn'] )
					#print a
					errors.append(a)
			...
			for element in line['het_lst']:
				if (element['strt_dte'] > datetime.date( year, 10, 1)) and (element['stts'].lower() != 'Approved'.lower() ):
					a = u'Сотрудник {name} запланировал свой отпуск после 1 октября. Требует дополнительного согласования.'.format( name = line['empl_ru_shrt_nme'])
					#print a
					errors.append(a)
			...
			for element in line['het_days_lst']:
				if line['het_days_lst'][element] >1:
					a = u'{name} допустил ошибку при формировании отпуска. Более одного het создано на {day}.'.format( name = line['empl_ru_shrt_nme'], day = element)
					errors.append(a)
		...
		for line_t in het_lst_by_day:
			...
			for lctn in lctn_lst:
				if ( lctn['lctn_nme'].lower() != u'Москва'.lower() ) and (( lctn['lnth_clng'] > 1 ) or ( lctn['lnth_ups'] > 1 )) :
					a = u'{day} В городе {lctn} превышено допустимое значение инженеров, одновременно находящихся в отпуске : '.format (lctn = lctn['lctn_nme'], day = line_t['day'])
					#print a
					errors.append(a)
					for persone in lctn['persone']:
						a = u'{persone_nme}-{stts}-{het} {dte} - {nts}'.format(persone_nme = persone['persone_nme'], stts = persone['stts'], het = persone['het'], dte = persone['entr_dte'], nts = persone['nts'] )
						#print a
						errors.append(a)
		#print '-----------------------------------'
		...
		for line_t in het_lst_by_day:
			...
			for het in line_t['persone_lst']:
				for line_f in fnctn_lst:
					if (het['empl_srs_lctn'].lower() == u'Москва'.lower()) and (line_f['fnctn_nme'].lower() == het['persone_tpe'].lower() ):
						line 				= {}
						line['persone_nme']	= het['empl_ru_shrt_nme']
						line['persone_tpe']	= het['persone_tpe']
						line['stts']		= het['stts']
			...
			#print '    ', fnctn_lst
			#for line in fnctn_lst:
				#print len(line['persone'])
			
			for fnctn in fnctn_lst:
				if ( fnctn['fnctn_nme'].lower() == 'SpetialCompany'.lower() ) and ( len(fnctn['persone']) > 2 ) :
					a = u'{date} В Москве превышено допустимое значение инженеров, одновременно находящихся в отпуске по SpetialCompany :'.format(date = line_t['day'])
					#print a
					errors.append(a)
				...
				if ( fnctn['fnctn_nme'].lower() == 'Cooling'.lower() ) and ( len(fnctn['persone']) > 2 ) :
					a = u'{date} В Москве превышено допустимое значение инженеров, одновременно находящихся в отпуске по Cooling (возможно 2 человека, запланировали {persone_len} человек):'.format(date = line_t['day'], persone_len = fnctn['persone_len'])
					#print a
					errors.append(a)
				...
				if (fnctn['fnctn_nme'].lower() == 'UPS'.lower() ) and (len(fnctn['persone']) > 3) :
					a = u'{date} В Москве превышено допустимое значение инженеров, одновременно находящихся в отпуске по UPS (возможно 3 человека, запланировали {persone_len} человек):'.format(date = line_t['day'], persone_len = fnctn['persone_len'])
					#print a
					errors.append(a)
		...
		for line_t in het_lst_by_day:
			...
			for fnctn in fnctn_lst:
				if (fnctn['fnctn_nme'].lower() == 'Cooling'.lower() ) and (len(fnctn['persone']) > 3) :
					a = u'{date} Превышено допустимое значение инженеров, одновременно находящихся в отпуске по Cooling (возможно 3 человека, запланировали {persone_len} человек):'.format(date = line_t['day'], persone_len = fnctn['persone_len'])
					#print a
					errors.append(a)
				...
				if (fnctn['fnctn_nme'].lower() == 'UPS'.lower() ) and (len(fnctn['persone']) > 7) :
					a = u'{date} Превышено допустимое значение инженеров, одновременно находящихся в отпуске по UPS (возможно 7 человек, запланировали {persone_len} человек):'.format(date = line_t['day'], persone_len = fnctn['persone_len'])
					#print a
					errors.append(a)
...
def make_chart( errors, lctn_el_lst ) :
	...
	try: 
		...
		for row_index in range(1, len(het_lst_by_day) + 4):
			ws_b.cell( row = row_index, column = frst_clmn).border = Border( right = Side( border_style='double', color=colors.BLACK ))
				
		if frst_clmn != lst_clmn :
			ws_b.merge_cells( '{strt_col}1:{end_col}1'.format ( strt_col = get_column_letter(frst_clmn), end_col = get_column_letter(lst_clmn) ) )
		...
		for row_index in range(4, 4 + len(het_lst_by_day) ) :
			#print str(ws_b.cell(column = 1, row = row_index).value), '\n'
		...
		for lctn in lctn_el_lst:
			...
			if frst_clmn != lst_clmn + 1:
				ws_c.merge_cells( '{strt_col}1:{end_col}1'.format ( strt_col = get_column_letter(frst_clmn), end_col = get_column_letter(lst_clmn) ) )
